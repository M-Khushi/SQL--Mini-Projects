...
SELECT bidder_id, count(bidder_id) total_bidders,
sum(if(bid_status='won',1,0)) as matches_won, ((sum(if(bid_status='won',1,0))/count(bidder_id))*100) as percentage_of_wins 
...
select s.stadium_name, s.city, count(ms.schedule_id) as no_of_matches
...
select s.stadium_name, count(m.match_id) as no_of_matches,
sum(if(m.toss_winner=m.match_winner,1,0)) as no_of_teams_won_match_and_toss,
(sum(if(m.toss_winner=m.match_winner,1,0))/count(m.match_id))*100 as percentage_of_teams_won_match_and_toss
from ipl_stadium s join ipl_match_schedule ms
on s.stadium_id=ms.stadium_id
...
select bd.bid_team ,t.team_name, COUNT(no_of_bids) total_bids
...
select M.MATCH_ID,T.TEAM_ID,WIN_DETAILS ,SUBSTRING_INDEX( SUBSTRING_INDEX(WIN_DETAILS," ",2)," ",-1)  SHORT_TEAM 
...
select t.team_name,sum(ts.matches_played) total_matches_played,
sum(ts.matches_won) total_matches_won, sum(ts.matches_lost)
...
select t.team_name,count(tp.player_role) no_of_allrounders
from ipl_team t join ipl_team_players tp
on t.team_id = tp.team_id
...
select  t.team_name, bd.BID_STATUS,bd.bidder_id, year(bd.bid_date) year_of_bid_date, sum(bp.total_points) total_bidder_points, s.stadium_name
from ipl_bidding_details bd join ipl_bidder_points bp
on bd.bidder_id= bp.bidder_id
join ipl_team t
on t.team_id = bd.bid_team
...
where bd.BID_STATUS = 'won' and bd.bid_team = 1 and s.stadium_name = 'M. Chinnaswamy Stadium'
group by bd.bidder_id
order by year_of_bid_date;
...
select t.team_name, p.player_name, tp.player_role,convert(substring_index(substring_index(substring_index(p.performance_dtls,' ',3),' ',-1),'-',-1),float) as wkt,
dense_rank() over(order by convert(substring_index(substring_index(substring_index(p.performance_dtls,' ',3),' ',-1),'-',-1),float) desc)
...
select t.bidder_id,(t.total_toss_wins/t.total_bids) *100 win_percentage 
from(
select *, sum(if(tw.toss_winners=tw.bid_team,1,0))total_toss_wins, count(*) total_bids
from(
select bd.bidder_id,bd.bid_team,
if(m.toss_winner=1,m.team_id1,m.team_id2) toss_winners
from ipl_match m join ipl_match_schedule ms
on m.match_id = ms.match_id
join ipl_bidding_details bd
on ms.schedule_id=bd.schedule_id) tw
group by tw.bidder_id) t
...
select t1.tournmt_id, t1.tournmt_name, if(t1.days=min(t1.days) over(order by t1.days asc), '"MINIUM DURATION','MAXIMUM DURATION') duration, t1.days duration_in_days
from(
select t.tournmt_id, t.tournmt_name,t.days,
if(days=first_value(t.days) over(order by days desc) or (days=first_value(days) over(order by days asc)),t.tournmt_name,null) min_max
from(
select *,datediff(to_date,from_date) days
from ipl_tournament)t
 )t1 where min_max is not null;
...
select distinct bd.bidder_id, ibd.bidder_name, year(bd.bid_date),month(bd.bid_date), 
sum(bp.total_points) over(partition by bd.bidder_id,year(bd.bid_date), month (bd.bid_date)) total_monthwise,
sum(bp.total_points) over(partition by bd.bidder_id, year(bd.bid_date)) total_points
from ipl_bidder_details ibd join ipl_bidding_details bd
on ibd.bidder_id = bd.bidder_id 
join ipl_bidder_points bp
on ibd.bidder_id = bp.bidder_id
where year(bd.bid_date)=2017
order by total_points desc, total_monthwise asc;

#14 Write a query for the above question using sub queries by having the same constraints as the above question.

 With sub as(
select distinct bd.bidder_id, ibd.bidder_name, year(bd.bid_date) year,month(bd.bid_date), 
sum(bp.total_points) over(partition by bd.bidder_id,year(bd.bid_date), month (bd.bid_date)) total_monthwise,
sum(bp.total_points) over(partition by bd.bidder_id, year(bd.bid_date)) total_points
from ipl_bidder_details ibd join ipl_bidding_details bd
on ibd.bidder_id = bd.bidder_id 
...
*/
SELECT BIDDER_ID,RNK,TOTAL_POINTS,IF(HIGHEST_3_BIDDERS IS NULL ,LOWEST_3_BIDDERS,HIGHEST_3_BIDDERS) HIGHEST_LOWEST_BIDDERS FROM (
SELECT T1.BIDDER_ID,T1.RNK,T1.TOTAL_POINTS,
IF(RNK<=3,IBP.BIDDER_NAME,NULL) HIGHEST_3_BIDDERS,IF(RNK>=21 ,IBP.BIDDER_NAME,NULL) LOWEST_3_BIDDERS 
FROM
(
SELECT DISTINCT *,DENSE_RANK() OVER(ORDER BY T.TOTAL_POINTS DESC) RNK  FROM (
SELECT BD.BIDDER_ID,SUM(BP.TOTAL_POINTS) OVER(partition by BD.BIDDER_ID) TOTAL_POINTS
FROM ipl_bidding_details BD JOIN ipl_bidder_points BP
ON BD.BIDDER_ID=BP.BIDDER_ID
WHERE YEAR(BD.BID_DATE)=2018)T ) T1
JOIN IPL_BIDDER_DETAILS IBP
ON T1.BIDDER_ID=IBP.BIDDER_ID
WHERE RNK<=3 OR RNK>=21)t2; 
